	...
	if (empty($uname)) {
		...
		if (mysqli_num_rows($result) === 1) {
            ...
            if($row['username'] === $uname){
				...
				header("Location: ../admin.php?error=Incorect password");
		        exit();
			}
            }else{
			...
			header("Location: ../admin.php?error=Username not Exist.");
	        exit();
		}
	}
	
}else{
	header("Location: ../admin.php?error=Username/Password is Required");
	exit();
}